...
python train_baseline.py --img 640 --batch 16 --epochs 5 --data coco128.yaml --weights yolov5s.pt
```
...
```bash
env COMET_MAX_IMAGE_UPLOADS=200 python train_baseline.py \
  --img 640 \
  --batch 16 \
  --epochs 5 \
  --data coco128.yaml \
...
```bash
env COMET_LOG_PER_CLASS_METRICS=true python train_baseline.py \
  --img 640 \
  --batch 16 \
  --epochs 5 \
  --data coco128.yaml \
